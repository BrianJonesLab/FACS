...
findPeaks1 <- function(file,tolerance=.995,peaks=5,plot=F) {
  resol=250
...
#kern2d creates a matrix were x are rows....now peace reigns again#
  ...
  if(length(na.exclude(local_maximas_x))<peaks) {
    warning(paste('only',length(na.exclude(local_maximas_x)),' peaks were higher that the tolerance value'))
...
findPeaks1(file = FACSC20141216_files[[3]],tolerance = .995,peaks = 5,plot = T)
